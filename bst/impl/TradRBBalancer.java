...
public class TradRBBalancer<K extends Comparable<K>, V> extends RBBalancer<K, V>  {
	public BSTMap<K, V, RBInfo<K, V>>.Node putFixup(BSTMap<K, V, RBInfo<K, V>>.Node fix) {
		...
		RBInfo<K,V> rightChild = fix.getRight().getInfo();
		RBInfo<K,V> leftChild = fix.getLeft().getInfo();
		// add code here
		
		
		//-------------------
		if (leftChild.isRed() && leftChild.isRedR() && rightChild.isRed()) { //Left-Right Red Uncle
			leftChild.blacken();
			rightChild.blacken();
			info.redden();
		}
		//-------------------
		else if(leftChild.isRed() && leftChild.isRedL() && rightChild.isRed()) { //Left-Left Red Uncle
			leftChild.blacken();
			rightChild.blacken();
			info.redden();
		}
		else if (leftChild.isRed()  && leftChild.isRedR() && !rightChild.isRed()) { //Left-Right Black Uncle
			fix.leftRotateLeft();
			info.redden();
			fix.getLeft().getInfo().blacken();
			fix = fix.rotateRight();
		}
		else if (leftChild.isRed()  && leftChild.isRedL() && !rightChild.isRed()) { //Left-Left Black Uncle
			info.redden();
			leftChild.blacken();
			fix = fix.rotateRight();
		}
		...
		else if (rightChild.isRed() && rightChild.isRedL() && leftChild.isRed()) { //Right-Left Red Uncle
			rightChild.blacken();
			leftChild.blacken();
			info.redden();
		}
		else if(rightChild.isRed() && rightChild.isRedR() && leftChild.isRed()) { //Right-Right Red Uncle
			rightChild.blacken();
			leftChild.blacken();
			info.redden();
		}
		else if (rightChild.isRed()  && rightChild.isRedL() && !leftChild.isRed()) { //Right-Left Black Uncle
			fix.rightRotateRight();
			info.redden();
			fix.getRight().getInfo().blacken();
			fix = fix.rotateLeft();
		}
		else if (rightChild.isRed()  && rightChild.isRedR() && !leftChild.isRed()) { //Right-Right Black Uncle
			info.redden();
			rightChild.blacken();
			fix = fix.rotateLeft();
		}
